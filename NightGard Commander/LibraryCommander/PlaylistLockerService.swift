...
final class PlaylistLockerService {
    ...
    /// Merged into NightGard Commander 2026-09-16. Commander is NOT sandboxed and has no
    /// iCloud container of its own, so `url(forUbiquityContainerIdentifier:)` returns nil.
    /// The backups already live in Library Commander's container folder in iCloud Drive,
    /// so the locker writes there by path — existing backups stay where they are.
    var lockerURL: URL? {
        let container = fileManager.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Mobile Documents/iCloud~com~NightGard~NightGard-Library-Commander", isDirectory: true)
        guard fileManager.fileExists(atPath: container.path) else { return nil }
        return container.appendingPathComponent("Documents/Playlist Locker", isDirectory: true)
    }
